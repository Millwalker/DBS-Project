...
for r in original['records']:
    ...
    r['year']=int(r['year'])
"""
#aus dem akuellen Datensatz ergänzen
new_data={'records' : []}
print(new_data)

num_data=len(data['records'])
num_orig=len(original['records'])
print(num_data,num_orig)
...
while(j<num_orig):
    if (i==num_data):
        while j<num_orig:
            tmp_orig=original['records'][j]
            new_data['records'].append(tmp_orig)
            j+=1
        break
    tmp_data=data['records'][i]
    tmp_orig=original['records'][j]
    if (tmp_data['dateRep'] != tmp_orig['dateRep']):
        new_data['records'].append(tmp_orig)
        j+=1
    else:
        new_data['records'].append(tmp_data)
        i+=1
        j+=1
print(len(new_data['records']))
print('last entry'+str(new_data['records'][3]))
print(original['records'][num_orig-1])

c=0
for i,j in zip(original['records'],new_data['records']):
    if i['dateRep']==j['dateRep']:
        c+=1
print("Anzahl der übereinstimmenden Einträge: "+ str(c))
"""

countries={}
"""
for i in new_data['records']:
    count=i['geoId']
    if count in countries:
        #countries[count]+=1
        pass
    else:
        countries[count]=i['countriesAndTerritories']
"""
...
for key in no_countries:
    print(ocountries[key])

print(len(original['records']))
...
#with open('test.json', encoding='utf-8-sig') as f: #'utf-8-sig'
#    df = pd.read_json(f)
